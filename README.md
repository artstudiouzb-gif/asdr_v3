...
**Контент**
- Конструктор страниц: 37 типов блока (hero, слайдеры, галереи, FAQ,
...
- Дизайн-система: цветовые палитры, Google-шрифты (20 наборных семейств
  плюс 4 рукописных для выделения в заголовках) или
...
├── templates/
│   ├── blocks/            # шаблоны блоков конструктора (35 файлов на 37 типов:
│   │                      # у контейнеров columns/tabs рендер программный)
...
**Новый тип блока объявляется один раз** — схемой полей
(`App\Core\BlockData\BlockFieldSchema`): из описания поля получаются
умолчание, поле формы, нормализация присланного и приведение сохранённых
данных на выводе, поэтому шаблон читает `$data` как есть. Плюс шаблон в
`templates/blocks/` и строка в `BlockTypeRegistry::BASE_DEFAULTS` (она задаёт
порядок типов в редакторе). Раньше то же знание лежало в четырёх местах —
реестр, форма, `collectData()`, шаблон — и списки значений расходились молча;
переезд типов на схему идёт постепенно, подробности в
...
Нативный тест-раннер без PHPUnit регистрирует **1781 тестовых сценариев**;
без переменных `TEST_DB_*` часть из них помечается пропущенной, а не молча
...
Новые JPEG/PNG автоматически получают WebP-варианты 400/800/1600 плюс полный
размер (ширины объявлены один раз — `Media::VARIANT_WIDTHS`). Для файлов,
...
| Документ | Что внутри |
|----------|------------|
| [docs/DEV.md](docs/DEV.md) | локальный запуск, проверки, как добавить тип блока |
| [docs/BLOCKS.md](docs/BLOCKS.md) | справочник всех 37 типов блоков и общих настроек секции |
