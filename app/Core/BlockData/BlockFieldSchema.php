...
final class BlockFieldSchema
{
    ...
    public static function all(): array
    {
        return self::$cache ??= [
            ...
            'form' => [
                ...
                // публичный шаблон, и второй список разъехался бы с первым.
                'layout' => Field::enum(
                    'Сетка формы',
                    \App\Core\FormLayout::COLUMNS,
                    '1col',
                    'Ширина каждого поля задаётся в самой форме («Формы» → конструктор): поле может занять половину, треть или весь ряд. Сетка задаёт, сколько это «как в сетке».'
                )->variants([
            ...
            'tabs' => [
                ...
                ->variants([
                    'segmented' => ['row:3+frame', 'Вкладки кнопками в общей дорожке — сразу видно, что выбор есть'],
                    'underline' => ['row:3', 'Вкладки текстом, активная с чертой — спокойнее для длинных названий'],
                    'vertical' => ['split', 'Список вкладок слева, содержимое справа — когда вкладок больше пяти'],
                ]),
            ...
            'partners' => [
                ...
                ->variants([
                    'row' => ['row:4', 'Логотипы стоят сеткой и не двигаются'],
                    'marquee' => ['track:4', 'Логотипы едут непрерывно — помещается любое их число'],
                ]),
            ...
            'faq' => [
                ...
                'panel' => Field::enum('Подложка вопросов', [
                    ...
                    'none' => 'Без подложки — вопросы разделяет линейка',
                ], 'auto', 'Вопросы и ответы — это текст: подложка под каждым вопросом читается как список кнопок.'),
            ...
            'docs_list' => [
                'variant' => Field::enum('Вариант списка', [
                    ...
                    'links' => 'Список ссылок',
                    'acts' => 'Правовые акты',
                    'acts-editorial' => 'Акты, редакционный вид',
                ...
                ->variants([
                    'grid' => ['grid:3+icon', 'Плитки со значком формата и размером файла'],
                    'links' => ['list:4+icon-left', 'Компактные строки — когда документов много'],
                    'acts' => ['grid:2+num', 'Карточки с номером и датой документа'],
                    'acts-editorial' => ['grid:2+num+accent', 'То же, но без подложки — для страниц со сплошным текстом'],
                ]),
                'title' => Field::text('Заголовок, показываемый на сайте')->named('title_field'),
            ...
            // «Хронология» и «Этапы» — один тип: оба показывают события во
            // времени. Различает их раскладка, и она стала настройкой, а не
            // вторым блоком: лента карточек с прокруткой или вертикальный
            // список с карточкой-призывом сбоку.
            'stages' => [
                'layout' => Field::enum('Раскладка', [
                    'track' => 'Лента карточек',
                    'list' => 'Вертикальный список',
                ], 'track', 'Лента годится для плана работ, список — для истории организации: событий много, и каждое короткое.')
                ->variants([
                    'track' => ['track:3', 'Этапы в ряд, в узком месте — прокрутка вбок'],
                    'list' => ['timeline:3', 'Годы слева, события справа — для длинной хронологии'],
                ])
                    ->group('Вид'),
                'variant' => Field::enum('Оформление ленты', [
                    'default' => 'Этапы реализации',
                    'history' => 'История организации',
                ], 'default')
                ->variants([
                    'default' => ['track:3', 'С метками состояния: завершён, в работе, запланирован'],
                    'history' => ['track:3+accent', 'Без меток состояния: линия сверху и больше воздуха'],
                ])
                    ->onlyWhen('layout', ['track'])
                    ->group('Вид'),
                'columns' => Field::intChoice(
                    'Этапов в ряду',
                    [0, 2, 3, 4, 5],
                ...
                'all_url' => Field::url('Ссылка «Все …» — URL'),
                // Карточка-призыв принадлежит вертикальному виду: в ленте её
                // некуда поставить — там карточки идут в ряд и она стала бы
                // шестым этапом без года.
                'cta_title' => Field::text('Заголовок карточки (пусто — карточки нет)')
                    ->onlyWhen('layout', ['list'])
                    ->group('Карточка рядом со списком'),
                'cta_text' => Field::textarea('Текст карточки')
                    ->onlyWhen('layout', ['list'])
                    ->group('Карточка рядом со списком'),
                'cta_button_text' => Field::text('Кнопка — текст')
                    ->onlyWhen('layout', ['list'])
                    ->group('Карточка рядом со списком'),
                'cta_button_url' => Field::url('Кнопка — ссылка')
                    ->onlyWhen('layout', ['list'])
                    ->group('Карточка рядом со списком'),
                'cta_image' => Field::media('Фоновое фото карточки')
                    ->onlyWhen('layout', ['list'])
                    ->group('Карточка рядом со списком'),
            ],
            'projects_list' => [
                ...
                ->variants([
                    'grid' => ['grid:3+photo', 'Карточки с фотографией рядами'],
                    'list' => ['list:3+photo-left', 'Фото слева, текст справа — помещается больше описания'],
                    'carousel' => ['track:3+photo', 'Прокрутка вбок со стрелками'],
                ]),
                'title' => Field::text('Заголовок, показываемый на сайте')->named('title_field'),
            ...
            'subscribe' => [
                ...
                ->variants([
                    'band' => ['band', 'Форма во всю ширину секции'],
                    'card' => ['card', 'Форма в карточке по центру'],
                    'image' => ['card+photo', 'Карточка поверх фонового кадра'],
                ]),
            ...
            'contact_cards' => [
                'variant' => Field::enum('Вариант отображения', [
                    'cards' => 'Иконка сверху',
                    'inline' => 'Иконка в строке',
                ], 'cards')
                ->variants([
                    'cards' => ['grid:3+icon', 'Иконка над заголовком — карточка выше'],
                    'inline' => ['grid:3+icon-left', 'Иконка рядом с заголовком — в ряд влезает больше'],
                ]),
                'title' => Field::text('Заголовок, показываемый на сайте')->named('title_field'),
                'panel' => Field::enum('Подложка карточек', [
                    'auto' => 'Как в «Дизайне»',
                    'card' => 'Карточка — фон, рамка и тень',
                    'none' => 'Без подложки — контакты прямо на фоне секции',
                ], 'auto', 'Карточка контакта сама никуда не ведёт: ведут телефон и почта внутри неё.'),
            ...
            'icon_text' => [
                ...
                ->variants([
                    'cards' => ['grid:3+icon', 'Каждый пункт в рамке с подложкой'],
                    'plain' => ['grid:3+icon+plain', 'Иконка и текст прямо на фоне секции'],
                    'inline' => ['list:4+icon-left', 'Короткие реквизиты строками — телефоны, адреса, режим работы'],
                ]),
                'title' => Field::text('Заголовок, показываемый на сайте')->named('title_field'),
                ...
                // Подпись пары — мелкая строка над значением, и до сих пор её
                // вид был константой в теме: в варианте «Без рамок» рядом с
                // крупным значением она читается как случайный серый текст, а
                // сделать из неё метку рубрики было нечем.
                'label_badge' => Field::bool(
                    'Подпись плашкой',
                    false,
                    'Подпись получает подложку тоном карточки — годится для рубрик и коротких меток. Без плашки остаётся мелкая строка над значением.'
                )
                    ->group('Подпись'),
                // Числом, а не пресетом «мельче / крупнее»: нужный кегль
                // зависит от длины подписи и от того, насколько крупное
                // значение стоит под ней. Ноль — кегль темы, поэтому у
                // собранных блоков вид не меняется.
                // Потолок равен кеглю значения (18px): при 24px подпись становилась
                // крупнее того, что подписывает, и иерархия переворачивалась —
                // помешать этому было нечем.
                'label_size' => Field::int(
                    'Размер подписи, px',
                    0,
                    18,
                    0,
                    '0 — кегль темы («Дизайн» → «Метаданные и подписи»).'
                )
                    ->group('Подпись'),
                'label_gap' => Field::int(
                    'Отступ подписи от значения, px',
                    0,
                    32,
                    0,
                    '0 — отступ темы. В строчном режиме это промежуток между подписью и значением в строке.'
                )
                    ->group('Подпись'),
            ],
            'cta' => [
            ...
            'counters' => [
                ...
                'variant' => Field::enum('Вид блока', [
                    'row' => 'Полоса',
                    'cards' => 'Карточки',
                    'progress' => 'Полоса к цели',
                    'scale' => 'Шкала по годам',
                ], 'row')
                ->variants([
                    'row' => ['row:4', 'Числа в ряд с разделителями, без подложки'],
                    'cards' => ['grid:4', 'Каждое число в своей карточке'],
                    'progress' => ['goal', 'Число, цель и полоса пройденного пути; у показателя задаётся цель'],
                    'scale' => ['axis', 'База, текущее значение и цель точками на одной линии — по годам'],
                ])
                    ->group('Вид блока'),
                ...
                ], 'auto', 'Без подложки блок годится и для одного показателя: остаётся иконка с числом, а фон берётся от секции.')
                    ->group('Вид блока'),
            ...
            'collage' => [
                'title' => Field::text('Заголовок, показываемый на сайте')->named('title_field'),
                // Тип сборки объявлен списком в CollageLayout: подписи, ключи
                // и сама раскладка — одно знание, и второй список здесь
                // разъехался бы с ним при первом же новом типе.
                'layout' => Field::enum(
                    'Тип сборки',
                    \App\Core\CollageLayout::LAYOUTS,
                    \App\Core\CollageLayout::FREE,
                    'Готовая сборка сама считает сетку и места элементов по их числу: добавили элемент — композиция перестроилась. Свободная сетка оставляет номера ячеек за вами.'
                )->variants([
                    'free' => ['grid:4', 'Номера ячеек задаёте сами — любое размещение и наложение'],
                    'strip' => ['row:3', 'Элементы одной высоты в ряд; хвост растягивается, дыры в ряду не остаётся'],
                    'hero' => ['hero-tiles', 'Крупный кадр держит взгляд, остальные идут спутниками'],
                    'mosaic' => ['mosaic', 'Пары «широкий + узкий» со сменой стороны — ряд не читается таблицей'],
                    'stack' => ['overlap', 'Элементы заходят друг на друга по диагонали; кто ниже в списке, тот сверху'],
                ]),
            ...
            'divider' => [
                ...
                ->variants([
                    'line' => ['rule', 'Черта во всю ширину — делит части текста'],
                    'short' => ['rule+dot', 'Черта по центру — мягче полной линии'],
                    'emblem' => ['emblem', 'Фирменная эмблема между двумя линиями'],
                    'space' => ['space', 'Только отступ, без линии и знака'],
                ]),
            ...
            'chart' => [
                ...
                ->variants([
                    'bars' => ['bars', 'Сравнение величин между собой'],
                    'stacked' => ['stacked', 'Одна полоса, разбитая на части: сумма — целое'],
                    'meter' => ['meter', 'Насколько выполнен один показатель'],
                ]),
                'rows' => Field::textarea(
                    'Данные',
            ...
            'table' => [
                ...
                ->variants([
                    'lines' => ['table', 'Тонкие черты между рядами'],
                    'striped' => ['table+striped', 'Через ряд подложка — легче вести глазом по длинной строке'],
                    'bordered' => ['table+bordered', 'Границы у каждой ячейки — для плотных числовых таблиц'],
                ]),
            ...
            'cards_grid' => [
                ...
                'description' => Field::richtext(
                    'Описание раздела',
                    'Заголовок и описание выводятся над карточками — отдельный текстовый блок не нужен.'
                )
                    ->group('Содержимое'),
                ...
                'variant' => Field::enum('Вариант карточек', [
                    ...
                    'compact' => 'Категории',
                    'image' => 'С фотографией',
                    'image_below' => 'Фото сверху',
                    'band' => 'Полоса',
                ], 'icon')
                ->variants([
                    'icon' => ['grid:3+icon', 'Иконка, заголовок и описание в карточке'],
                    'compact' => ['grid:6', 'Мелкие плитки с одним названием'],
                    'image' => ['grid:3+photo', 'Заголовок поверх кадра'],
                    'image_below' => ['grid:3+photo-below', 'Кадр, под ним заголовок и текст'],
                    'band' => ['band+icon', 'Одна тёмная полоса на всю ширину вместо карточек'],
                ])
                ...
                ->variants([
                    'auto' => ['auto', 'Сетка, а если карточек больше, чем колонок, — прокрутка'],
                    'grid' => ['grid:4', 'Карточки переносятся рядами, прокрутки нет'],
                    'slider' => ['track:3', 'Всегда полоса со стрелками, в один ряд'],
                ])
                    ->group('Вид и раскладка'),
                'columns' => Field::intChoice(
                    'Колонок',
                ...
                'numbering' => Field::bool(
                    'Нумеровать карточки',
                    true,
                    'Номер 01, 02… в углу карточки. Раньше он печатался всегда, и выключить его было нечем.'
                )
                ...
                ->variants([
                    'old' => ['grid:3+icon+frame', 'Рамка, подложка и тень'],
                    'new' => ['grid:3+icon+accent', 'Линия сверху, без подложки — легче на плотной странице'],
                ])
                    ->onlyWhen('variant', ['icon'])
                    ->group('Карточка с иконкой'),
                'card_gap' => Field::int(
                ...
                'icon_position' => Field::enum('Положение иконки', [
                    ...
                    'right' => 'Справа от текста',
                    'center' => 'Сверху по центру',
                    // Пришло из «Преимуществ»: там это был отдельный вариант
                    // блока, хотя меняет он ровно положение иконки.
                    'inline' => 'В строке с заголовком',
                ], 'top', 'Слева и справа — рядом с текстом; по центру — над заголовком; в строке — иконка и заголовок на одной линии, текст под ними.')
                    ->onlyWhen('variant', ['icon'])
                    ->group('Карточка с иконкой'),
            ...
            'news_feature' => [
                ...
                ->variants([
                    'cards' => ['split+photo', 'Одна большая новость, рядом список остальных'],
                    'mosaic' => ['mosaic', 'Ритм ленты: крупные с текстом на фото и компактные вперемешку'],
                    'columns' => ['grid:3+photo', 'Равные карточки с фото над заголовком'],
                ]),
                'columns' => Field::intChoice(
                ...
                'limit' => Field::int(
                    ...
                    12,
                    5,
                    'Общее количество материалов. Для трёх колонок удобно 3, 6 или 9. «Мозаика» всегда запрашивает 6 материалов: столько занимает её ритм без обрыва ряда.'
                ),
                'all_text' => Field::text('Ссылка «Все …» — текст', 'Все новости', '', 'Все новости'),
            ...
            'map_point' => [
                ...
                'map_width' => Field::int(
                    ...
                    100,
                    100,
                    '100% — на всю ширину блока. При меньшем значении карта выравнивается по центру.'
                ),
                'map_height' => Field::int(
                    'Высота карты, px (0 — высота из темы)',
                    0,
                    1000,
                    0,
                    'Оставьте 0, чтобы старые страницы сохранили текущую высоту карты.'
                ),
                'map_width_mobile' => Field::int(
                    'Ширина карты на телефоне, %',
                    40,
                    100,
                    100
                ),
                'map_height_mobile' => Field::int(
                    'Высота карты на телефоне, px (0 — как на компьютере)',
                    0,
                    800,
                    0
                ),
            ...
            'org_structure' => [
                ...
                ->variants([
                    'tree' => ['tree', 'Ветки рядами под руководителем'],
                    'spine' => ['tree+spine', 'Ветки по обе стороны от вертикальной оси'],
                ]),
                'columns' => Field::intChoice(
                ...
                'font_size' => Field::intChoice(
                    'Размер текста схемы',
                    [0, 13, 14, 16, 18, 20, 22],
                    0,
                    'Схема бывает и на десяток узлов, и на сотню: крупной она читается с проектора, мелкой помещается на экран целиком. Размеры внутри — доли от этого числа; мельче читаемого схема не становится.',
                    [0 => 'Как в теме']
                ),
    ...
    public static function normalize(string $type, array $post, string $locale): array
    {
        ...
        foreach (self::fields($type) as $key => $field) {
            ...
            $data[$key] = match ($field->kind) {
                ...
                // Пустая строка приводится к нулю, а ноль бывает законным
                // значением набора («Колонок: автоматически»), поэтому наличие
                // присланного проверяется отдельно от набора: иначе форма без
                // этого поля читала бы ключ, которого нет.
                'int_choice' => is_scalar($post[$name] ?? null)
                    && isset($field->options[(int) (string) $post[$name]])
                        ? (int) $post[$name]
                        : (int) $field->default,
    ...
    private static function fieldHtml(string $key, Field $field, array $data): string
    {
        ...
        // Вариант отображения — плитками с рисунком раскладки, а не списком:
        // список отвечает на вопрос «как это будет выглядеть» словом, и чтобы
        // отличить «Мозаику» от «Колонок», редактор сохранял блок и открывал
        // страницу. Виджет приносит свой контейнер, поэтому условие показа
        // вешается обёрткой снаружи — как у медиаполя и пикера иконок.
        if ($field->kind === 'enum' && $field->variants !== []) {
            $widget = AdminUi::variantField(
                $name,
            ...
            return '<div class="bf-field bf-field--full"' . $when . '>' . $widget . '</div>';
        }
