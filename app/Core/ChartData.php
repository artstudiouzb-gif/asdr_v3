...
final class ChartData
{
    ...
    public static function parse(string $source, string $variant = 'bars', float $total = 0.0): array
    {
        ...
        if ($variant === 'stacked') {
            // Доля целого отрицательной не бывает: «−8 % от суммы» не значит
            // ничего, а в сумму такое значение входит со знаком и уменьшает
            // базу — соседние доли вырастают на пустом месте. Строка
            // отбрасывается так же, как строка без числа.
            $rows = array_values(array_filter($rows, static fn (array $row): bool => $row['value'] >= 0));
            if ($rows === []) {
                return ['rows' => [], 'total' => 0.0, 'max' => 0.0];
            }
        ...
        // Длина полосы — это ВЕЛИЧИНА, а не значение со знаком: у столбцов
        // база считается по наибольшему модулю. Иначе «Рост 12 / Спад −8»
        // давало базу 12 и долю −66.67 %, а `width: -66.67%` недопустим —
        // браузер отбрасывает объявление, и полоса берёт `auto`, то есть всю
        // ширину (замерено в Chromium: 400px, ровно как у 100 %). Спад
        // рисовался самым длинным столбцом диаграммы. Когда отрицательны все
        // значения, прежняя база была отрицательной, `$base > 0` не
        // выполнялось, и полос не рисовалось вовсе — подписи без полос.
        $maxAbs = $values === [] ? 0.0 : max(array_map('abs', $values));

        // База расчёта: у долей это сумма, у показателя к цели — 100 %,
        // у столбцов — самая большая величина (самая длинная полоса = вся ширина).
        $base = match (true) {
            $total > 0 => $total,
            $variant === 'stacked' => $sum,
            $variant === 'meter' => 100.0,
        ...
        // Доля зажата с обеих сторон: сверху — чтобы значение больше базы не
        // вылезало за полотно, снизу — чтобы в CSS никогда не уходило
        // отрицательное число. Знак величины показывает текст рядом с полосой
        // (`ChartData::formatNumber`), а не её длина.
        $rows = array_map(
            static fn (array $row): array => $row + [
                'share' => $base > 0 ? min(100.0, max(0.0, round(abs($row['value']) / $base * 100, 2))) : 0.0,
            ],
            $rows
        );
