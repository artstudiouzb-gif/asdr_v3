...
final class HeroRenderer
{
    ...
    private static function slideCss(array $d, array $s, string $scope, int $index, string $position): string
    {
        ...
        // Выбранный редактором цвет надписи главнее подбора — тот же порядок,
        // что у «Акцентной цитаты». Стоит после заливки: иначе пересчёт по
        // контрасту затирал бы явный выбор.
        if ($d['cta_text_color'] !== '') {
            $vars['--on-accent'] = (string) $d['cta_text_color'];
        }
