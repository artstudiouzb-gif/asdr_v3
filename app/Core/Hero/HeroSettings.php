...
final class HeroSettings
{
    ...
    /**
     * Подписи вариантов цвета для форм админки — обложки и слайда.
     *
     * Слова «светлый» и «тёмный» встречаются у обложки трижды: фон, буквы и
     * вуаль. Поэтому подпись называет то, что меняется, а не оттенок сам по
     * себе: иначе «Светлая» схема со «Светлым» текстом читается как
     * согласованная пара, а даёт белым по белому. Список один на обе формы —
     * второй разъехался бы с первым при первой правке; ключи обязаны
     * совпадать со SCHEMES и CONTENT_SCHEMES (стережёт тест 246).
     */
    public const SCHEME_LABELS = [
        'light' => 'Светлый фон',
        'dark' => 'Тёмный фон',
        'navy' => 'Тёмно-синий фон',
        'custom' => 'Свои цвета',
    ];
    public const CONTENT_SCHEME_LABELS = [
        'auto' => 'Автоматически — по фону',
        'light' => 'Светлый (белые буквы)',
        'dark' => 'Тёмный (почти чёрные буквы)',
    ];
    public const CONTENT_SCHEME_HINT = 'Цвет самих букв, а не фона: светлый — белые буквы для тёмного кадра, '
        . 'тёмный — почти чёрные для светлого. «Автоматически» смотрит на фон обложки и наложение.';
