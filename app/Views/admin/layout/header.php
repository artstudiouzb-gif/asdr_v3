...
// Медиатека стоит последней в группе: это библиотека, из которой берут
// снимки, а не раздел материалов рядом с новостями и альбомами.
$navContent['files'] = ['/admin/files', t('Медиафайлы')];

// В «Инструментах» остаётся то, чем настраивают сайт, а не то, что на нём
// публикуют: формы и библиотека шаблонов. Материалы — альбомы, видео,
// медиатека — переехали в «Контент», к новостям и страницам.
$navTools = [
    'forms' => ['/admin/forms', t('Формы')],
    'snippets' => ['/admin/snippets', t('Шаблоны страниц')],
];
// Корзина — административный раздел (право `manage_trash`), и пункт обязан
// спрашивать то же право, что и сам контроллер: иначе редактор видит в меню
// ссылку, которая отвечает ему 403.
if (\App\Core\RbacGuard::can('manage_trash')) {
    $navTools['trash'] = ['/admin/trash', t('Корзина')];
}
