...
<div class="form-card block-editor-card">
    <form method="post" action="/admin/blocks/<?= (int) $block['id'] ?>/edit" class="form-grid block-editor-form" data-content-draft="block:<?= (int) $block['id'] ?>" data-record-updated="<?= htmlspecialchars((string) ($block['updated_at'] ?? ''), ENT_QUOTES) ?>">
        ...
        <?php if ($type === 'form'): ?>
            ...
            <?= \App\Core\AdminUi::variantField(
                'columns',
                (string) \App\Core\FormLayout::columns($data),
                ['1' => 'В одну колонку', '2' => 'В две колонки', '3' => 'В три колонки'],
                [
                    '1' => ['list:4+frame', 'Поля идут друг под другом — привычно и работает на любом экране'],
                    '2' => ['grid:2+frame', 'Короткие поля встают парами — форма занимает вдвое меньше высоты'],
                    '3' => ['grid:3+frame', 'Плотный ряд для коротких полей: индекс, район, дата'],
                ],
                'Сетка формы'
            ) ?>
        ...
        <?php if ($type === 'hero'): ?>
            ...
            <div class="colorfield-row">
                <?= \App\Core\AdminUi::colorField('text_color', $data['text_color'] ?? '', 'Цвет текста', '#ffffff', 'Авто по фону') ?>
