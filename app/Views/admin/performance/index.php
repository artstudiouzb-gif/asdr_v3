...
<?php endif; ?>
        ...
        <div class="header-builder__group" id="perf-cloudflare">
            ...
            <p class="form-hint u-inline-291b7bbb01">
                ...
                он работает другими заголовками. Кнопка «Проверить связь» разбирает подключение по шагам —
                сам токен, доступ к зоне и пробную очистку по несуществующему адресу — и называет тот шаг,
                который не прошёл.
            </p>
            <p class="form-hint">
                Если очистка отказывает при живом токене — <code>Unable to purge. Unauthorized.</code> —
                причина не всегда в самом токене. <b>Токен не может больше своего хозяина:</b> когда зона
                лежит в чужом аккаунте Cloudflare, а ваша роль в нём урезана, право очистки срезается молча,
                как бы оно ни было записано в токене (Manage Account → Members). И зону, которую ведёт
                партнёр — хостинг или регистратор, — из своего аккаунта очистить нельзя вовсе. Проверка
                связи называет аккаунт зоны и партнёра, если он есть.
            </p>
