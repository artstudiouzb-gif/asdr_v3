...
`update_worker.php` — исполнитель кнопки «Обновление системы» в панели.
Панель обновление не выполняет и выполнять не может: веб-запрос обрывается по
таймауту, и обрыв посреди замены оставил бы половину старых файлов и половину
новых. Поэтому кнопка ставит задачу, а этот воркер делает работу из командной
строки. **Без этой строки в crontab кнопка в панели недоступна** — раздел
`/admin/update` покажет, что воркер не отвечает, и заказать обновление не даст:
записанное намерение всё равно некому было бы выполнить.

Он же прибирает за оборвавшимся обновлением: если процесс убили посреди замены,
следующий запуск снимает режим обслуживания и закрывает зависшее состояние.
Пока воркер молчит, сайт всё равно откроется сам — публичная точка входа не
показывает заглушку, включённую обновлением, которое перестало отчитываться.
...
### Автоматически, из релиза на GitHub (рекомендуется)

```bash
php scripts/update.php --check                  # что стоит и что вышло
php scripts/update.php --dry-run                # план: что заменится, что удалится
php scripts/update.php https://artstudio.uz     # обновить (спросит подтверждение)
```

Скрипт делает всю последовательность сам и **останавливается на первой же
неудаче**: узнаёт последний релиз → скачивает установочный архив и его
`.sha256` → сверяет сумму → проверяет состав архива → снимает **полную
резервную копию** и снимок заменяемых файлов → включает режим обслуживания →
заменяет файлы → накатывает миграции → пересобирает эталон целостности →
чистит кэш → выключает режим обслуживания → прогоняет `release_check` и
smoke-обход.

Что важно знать:

- **Данные сайта не трогаются**: `config/config.php`, `storage/` и
  `public/uploads/` в архиве отсутствуют и в план замены не попадают.
- **Устаревшие файлы удаляются** из каталогов, которыми владеет релиз
  (`app/`, `templates/`, `scripts/`, `database/`, `public/assets/`) — иначе
  после обновления продолжал бы отвечать старый обработчик. Всё удаляемое
  сначала копируется в снимок отката и печатается списком до подтверждения.
- **Ставится только собранный архив** `asdr-cms-*.zip` вместе с `.sha256`.
  «Source code» из GitHub не годится: в нём тесты, `.github` и `composer.json`.
  Релиз без такого архива — отказ с объяснением; соберите архив workflow
  «Release package» (пуш тега `v*`).
- **Сорвалась замена** — файлы возвращаются из снимка автоматически, режим
  обслуживания снимается. Сорвался шаг после замены (миграция) — восстановление
  из копии: `php database/restore.php <архив> <база> <каталог>`.
- **Окно недоступности** — секунды: режим обслуживания живёт в БД, и запрос,
  пришедший ровно в момент замены файлов, может получить ошибку. Обновляйтесь
  в непиковое время.
- Репозиторий берётся из `UPDATE_REPO` (по умолчанию `artstudiouzb-gif/asdr_v2`)
  — из окружения, а не из настроек в панели.

Кнопки «обновить» в админке нет намеренно: замена кода — самое опасное
действие в CMS, а веб-запрос обрывается по таймауту и может оставить сайт с
половиной старых и половиной новых файлов.
...
Для выпуска после ручной загрузки новой версии — fail-fast сценарий:
