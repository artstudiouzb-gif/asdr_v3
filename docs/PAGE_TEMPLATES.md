...
Обе кнопки есть в двух местах: в редакторе страницы (секция «Шаблоны страницы»
под списком блоков) и в разделе **«Шаблоны страниц»** (`/admin/snippets`,
группа «Инструменты»).

## Раздел «Шаблоны страниц»

Библиотека была с самого начала, но жила внутри формы страницы: увидеть
сохранённое можно было, только открыв какую-нибудь страницу и прокрутив её
конструктор до конца, а скачать файлом — тем более, список выгрузки стоял там
же. Раздел показывает её целиком:

- состав, вес снимка и дату у каждого шаблона; «Автокопия: …» помечена меткой;
- **скачивание файлом** — ссылка в строке;
- **загрузку из файла** — та же проверка, что и в редакторе;
- **переименование** — имя приезжает из файла или из старой сборки, и без
  правки на месте его пришлось бы выгружать и загружать заново;
- **применение** к любой странице или проекту (выбор записи, языка и режима)
  и **удаление**.

Применение — одна последовательность на редактор и на раздел
(`SnippetController::applySnippet`): вторая копия разъехалась бы при первой
правке — в одной появилась бы автокопия перед заменой, в другой нет. После
применения открывается конструктор выбранной записи: результат смотрят там.
...
- Раздел — `SnippetController::index/apply/rename`, вьюха
  `app/Views/admin/snippets/index.php`, маршруты `/admin/snippets*`.
