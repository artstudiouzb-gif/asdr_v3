(function () {
    ...
    // Мягкое появление фотографий: кадр проявляется из залитой подложки, а не
    // сменяет её рывком. Список мест объявлен один раз, и он же является
    // списком (тот же приём, что у GRIDS в появлении карточек); само правило
    // в теме никаких имён не знает и висит на классе `is-soft-media`.
    //
    // **Чего в списке нет и почему.** Главный кадр галереи на детальной
    // странице (`.newsdetail-gallery__slide`) — самый крупный элемент первого
    // экрана, то есть LCP страницы: спрятав его ради плавности, мы задержали бы
    // ровно то, ради чего страницу открыли (то же правило, по которому первый
    // блок страницы не получает появления при скролле). Вдобавок у слайдов
    // **уже** своя механика прозрачности — переключение кадров классом
    // `is-active`, — и второй механизм на том же свойстве спорил бы с ней.
    // Обложка видео не в списке по той же причине: она тоже первый экран.
    // Миниатюры галереи (110×62) и кадры соседних новостей (112px) пропущены
    // намеренно: на таком размере проявление не читается, а лишнее движение
    // под уже показанной фотографией — шум.
    var SOFT_MEDIA = [
        '.relnews-card__media img',
        '.newsdetail-photos__item img',
        '.newsdetail-sidegallery__item img'
    ].join(', ');
    ...
    (function () {
        ...
        imgs.forEach(function (img) {
            ...
            // Переход живёт на отдельном классе и остаётся после снятия
            // бледности; обоих классов нет у кадра, который уже готов.
            img.classList.add('is-soft-media');
        ...
        document.addEventListener('mousemove', function (event) {
            ...
            window.requestAnimationFrame(function () {
                ...
                var el = e.target.closest(
                    '.cat-tile, .contact-card, .project-card, .team-card, .feature-card, .news-card, .album-card, .doc-card, .act-card, .catcard, .testimonial, .mediacard, .imgcard, .faq-item, .stage, .timeline-item, ' +
    ...
    (function () {
        ...
        images.forEach(function (img) {
            ...
            var triggerTarget = function () {
                var gallery = img.closest('[data-ndgallery]');
                if (gallery) {
                    var activeImg = gallery.querySelector('.newsdetail-gallery__slide.is-active');
            ...
            trigger.addEventListener('click', function (e) {
                e.preventDefault();
                var targetImg = triggerTarget();
                modalImages = selectImageGroup(targetImg);
                var idx = modalImages.indexOf(targetImg);
                if (idx !== -1) {
                    showIndex(idx);
                    openModal(trigger);
                }
        ...
        document.querySelectorAll('[data-ndgallery] .newsdetail-gallery__main').forEach(function (main) {
            main.addEventListener('click', function (e) {
                if (e.target.closest('button, [data-ndg-prev], [data-ndg-next], [data-ndg-thumb], [data-ndg-counter]')) {
                    return;
                }
                var gallery = main.closest('[data-ndgallery]');
                var activeImg = (gallery ? gallery.querySelector('.newsdetail-gallery__slide.is-active') : null)
                    || main.querySelector('.newsdetail-gallery__slide');
                if (!activeImg) { return; }
                e.preventDefault();
                modalImages = selectImageGroup(activeImg);
                var idx = modalImages.indexOf(activeImg);
                if (idx !== -1) {
                    showIndex(idx);
                    openModal(activeImg);
                }
        ...
        var showIndex = function (idx) {
            ...
            if (gallery) {
                ...
                if (typeof gallery.__ndgShow === 'function' && galleryIndex !== -1) {
                    gallery.__ndgShow(galleryIndex);
                }
