...
/**
 * Права `RbacGuard`, у которых нет прямого потребителя, и чем они закрыты вместо
 * него.
 *
 * Право без потребителя — это запрет, которого не существует: матрица прав
 * читается как действующая, а раздел открыт. Именно так жило `manage_trash` —
 * объявлено административным наравне с `manage_users`, не проверялось нигде, и
 * роль `editor` доходила до безвозвратного удаления страниц и новостей.
 *
 * Но «нет вызова с этим именем» не всегда значит «не закрыто»: часть разделов
 * закрыта равносильной проверкой роли (`Auth::requireSuperAdmin()`), а часть
 * прав разрешена обеим ролям и потому не сужает ничего. Такие случаи
 * перечисляются здесь с причиной — цена входа одна строка, как у
 * ADMIN_MEDIA_ONLY_BY_DESIGN.
 *
 * @var array<string, string>
 */
const RBAC_PERMISSIONS_ENFORCED_ELSEWHERE = [
    'manage_users' => 'UserController закрыт Auth::requireSuperAdmin() — та же роль, что в матрице',
    'manage_settings' => 'SettingsController и соседние разделы закрыты Auth::requireSuperAdmin()',
    'manage_design' => 'DesignController закрыт Auth::requireSuperAdmin()',
    'publish_content' => 'разрешено обеим ролям: отдельной проверки не сужает ничего',
    'edit_content' => 'разрешено обеим ролям: отдельной проверки не сужает ничего',
];
...
const DESIGN_SCALE_CEILINGS = [
    ...
    // 13 -> 9: вместе с «Преимуществами» ушла копия их правил, где размеры
    // были записаны числами.
    'font-size' => 9,
    // 163 -> 161 -> 156 -> 154: последнее число снято отчётом по бюджетам,
    // потолок опущен под факт (запас — это разрешение деградировать).
    // 154 -> 77: нейтральный подъём сведён к ролям tokens.css (control,
    // card, hover, overlay, modal); остались кольца, свечения и цветные тени.
    'box-shadow' => 77,
    ...
    // отчёт по бюджетам это и показал. 320 -> 318 после слияния
    // «Преимуществ» с «Карточками».
    '!important' => 318,
];
