...
test('Admin assets: media workflow-слои входят в бандл админки', function (): void {
    $manifest = json_decode((string) file_get_contents(APP_ROOT . '/public/assets/asset-manifest.json'), true);
    assert_true(is_array($manifest));
    $js = $manifest['admin']['/assets/admin/admin.min.js']['sources'] ?? [];
    $css = $manifest['admin']['/assets/admin/admin-panel.min.css']['sources'] ?? [];

    assert_true(in_array('/assets/js/admin-workflow-fixes.js', $js, true));
    assert_true(in_array('/assets/js/admin-gallery-dropzone.js', $js, true));
    assert_true(in_array('/assets/js/admin-media-loadmore.js', $js, true));
    assert_true(in_array('/assets/css/admin-workflow-fixes.css', $css, true));
    assert_true(in_array('/assets/css/admin-media-unified.css', $css, true));
});
