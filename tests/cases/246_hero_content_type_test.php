...
test('Подписи цветовых вариантов покрывают все значения и называют, что меняется', function () {
    // Подписи лежат рядом со списками значений, но списками не являются:
    // разъехавшись, они дали бы вариант без подписи — пустую строку в
    // выпадающем списке формы.
    assert_same(HeroSettings::SCHEMES, array_keys(HeroSettings::SCHEME_LABELS), 'подписи фона покрывают схемы');
    assert_same(HeroSettings::CONTENT_SCHEMES, array_keys(HeroSettings::CONTENT_SCHEME_LABELS), 'подписи текста покрывают режимы');

    // Слово «светлый» в обложке значит три разные вещи (фон, буквы, вуаль),
    // поэтому подпись обязана называть предмет, а не один оттенок.
    assert_true(
        str_contains(HeroSettings::SCHEME_LABELS['light'], 'фон'),
        'подпись схемы говорит про фон'
    );
    assert_true(
        str_contains(HeroSettings::CONTENT_SCHEME_LABELS['light'], 'буквы'),
        'подпись цвета текста говорит про буквы'
    );
});
