...
/*
 * Отрицательная величина. Редактор набирает динамику показателя как в
 * документе — «Спад | -8», — и прежде это давало долю −66.67 %, которая
 * уходила в scoped CSS как `width: -66.67%`. Такое объявление недопустимо:
 * браузер отбрасывает его целиком, и полоса берёт `auto`, то есть всю ширину
 * (замерено в Chromium: 400px при полотне 400px — ровно столько же, сколько у
 * доли 100 %). То есть спад рисовался самым длинным столбцом диаграммы:
 * не «не нарисовался», а показал неправду.
 *
 * Длина полосы — это величина, знак показывает текст рядом с ней.
 */
test('Диаграмма: отрицательная величина рисуется длиной, а не уходит в CSS со знаком', function (): void {
    $chart = ChartData::parse("Рост | 12\nСпад | -8", 'bars');

    $shares = array_column($chart['rows'], 'share', 'label');
    assert_same(100.0, $shares['Рост'], 'наибольшая величина занимает полотно целиком');
    assert_same(66.67, $shares['Спад'], 'спад короче роста ровно во столько, во сколько меньше по величине');

    // Знак не теряется: его показывает подпись значения.
    assert_same('-8', ChartData::formatNumber(-8.0));

    // Когда отрицательны все значения, прежняя база была отрицательной и полос
    // не рисовалось вовсе — оставались подписи над пустым местом.
    $all = ChartData::parse("Янв | -3\nФев | -7", 'bars');
    assert_same(100.0, $all['rows'][1]['share'], 'наибольшая по модулю величина занимает полотно');
    assert_true($all['rows'][0]['share'] > 0, 'полоса обязана быть видимой');

    // Ни при каком варианте в CSS не уходит доля вне [0..100]: там она
    // становится недопустимым `width` и полоса берёт всю ширину.
    foreach (['bars', 'stacked', 'meter'] as $variant) {
        foreach (ChartData::parse("A | -5\nB | 10\nC | 0\nD | 250", $variant)['rows'] as $row) {
            assert_true(
                $row['share'] >= 0.0 && $row['share'] <= 100.0,
                'доля вне [0..100] у варианта ' . $variant . ': ' . $row['share']
            );
        }
    }
});

test('Диаграмма: доля целого отрицательной не бывает', function (): void {
    // В сумму отрицательное значение входит со знаком и уменьшает базу —
    // соседние доли вырастают на пустом месте. Строка отбрасывается так же,
    // как строка без числа.
    $chart = ChartData::parse("A | 5\nB | -5\nC | 10", 'stacked');

    assert_same(['A', 'C'], array_column($chart['rows'], 'label'));
    assert_same(15.0, $chart['total'], 'база считается по оставшимся долям');
    assert_same(33.33, $chart['rows'][0]['share']);

    // Одни отрицательные доли — это пустая диаграмма, а не деление на ноль.
    assert_same([], ChartData::parse("A | -5\nB | -10", 'stacked')['rows']);
});
