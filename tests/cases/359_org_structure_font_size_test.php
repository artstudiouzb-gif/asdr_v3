...
test('Размеры схемы считаются от --org-font, а не задаются пикселями', function () {
    ...
    // Потолок из шкалы обесценивал настройку: доля от 22px упиралась в
    // 15.2px, и разница с умолчанием была в пиксель. Ограничение осталось
    // только снизу — `max()`, а не `clamp()`.
    assert_same(
        0,
        preg_match_all('/clamp\([^)]*--org-font/', $css),
        'потолок размера вернулся: настройка снова почти не действует'
    );
