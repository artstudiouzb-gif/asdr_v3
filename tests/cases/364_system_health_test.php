...
test('Каждая известная интеграция где-то записывает свой исход', function (): void {
    ...
    // Вместе с подкаталогами: запрос к модели переехал в App\Core\Ai, и
    // проверка «только верхний уровень» перестала бы видеть запись исхода —
    // то есть молча ослабла бы ровно там, где код переложили.
    $sources = '';
    foreach (array_merge(glob(APP_ROOT . '/app/Core/*.php') ?: [], glob(APP_ROOT . '/app/Core/*/*.php') ?: []) as $file) {
