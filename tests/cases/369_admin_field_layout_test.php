...
test('Схема полей раскладывает настройки сеткой и группами', function (): void {
    ...
    // Вариант отображения рисуется плитками со схемой раскладки и поэтому
    // занимает строку целиком: в половине колонки плитки сжимаются до полосок,
    // а смотрят на них именно ради рисунка.
    assert_contains('class="bf-field bf-field--full"', $html, 'поле-вариант занимает всю строку');
    assert_contains('class="variant-field"', $html, 'вариант отображения рисуется плитками');
